...
> `level_bonus` — прибавка к уровню персонажа в УРОВНЯХ (столбец «Прибавка к уровню» листа «Гардероб»). При покупке бэк доначисляет ровно столько EXP, сколько нужно на эти уровни от текущего уровня игрока.
> `exp_bonus` — сырой EXP; применяется только если `level_bonus` не задан (скины, заведённые админом вручную). Уровень в обоих случаях остаётся производной от опыта, так что прогресс не рассинхронизируется.
...
- Покупка дома, кроме самого дома, даёт ещё и уровни персонажа: столбец «Прибавка к уровню» листа «Дом» (`house_levels.level_bonus`). То же и у скинов — `skins.level_bonus` (лист «Гардероб»).
- **Прибавка задаётся в УРОВНЯХ, а не в EXP.** Начисляется ровно столько опыта, сколько нужно на эти уровни от текущего (`WalletService.grantLevels`). Фиксированный курс тут не годится: порог уровня растёт с 500 EXP на 1-м до 10100 на 49-м, так что «+1 уровень» стоит очень по-разному в зависимости от того, где игрок сейчас.
- `skins.exp_bonus` остаётся для скинов, которым админ задаёт опыт напрямую — используется, только если `level_bonus` не задан.
