...
### Модель логина: пользователь придумывает его сам

Регистрация — самостоятельная, без модерации куратором. Ребёнок (или родитель за него) при регистрации **сам придумывает `login` и `password`**, тут же заполняя и остальные данные — аккаунт создаётся сразу активным, никакой очереди заявок больше нет.

1. `POST /api/auth/register` — одним запросом: ФИО ребёнка, класс, имя родителя, телефон родителя (контакт для связи, не учётные данные) **плюс** самостоятельно выбранные `login` и `password`. Аккаунт создаётся сразу (`status: active`).
2. Вход — `POST /api/auth/login` этой же парой `{ login, password }`, сразу после регистрации, без ожидания.

`login` должен быть уникален — при попытке занять чужой логин `POST /api/auth/register` вернёт `409 CONFLICT`. `parent_phone`/`parent_name` хранятся как контакт для связи и попадают в поиск (`GET /api/admin/users?search=`), но войти по ним нельзя — только по `login`.
...
{ "success": true, "data": { /* ... */ } }
```
Некоторые эндпоинты вместо `data` возвращают несколько полей на верхнем уровне (`message`, `userId`, `users`, `total` и т.п.) — это отмечено в описании каждого такого эндпоинта отдельно.
...
| 409 | `CONFLICT` | Дубликат (логин уже занят, скин уже куплен, уже отвечено сегодня) |
...
Регистрация без модерации — аккаунт создаётся сразу активным, можно логиниться следующим же запросом.
...
| parent_phone | string (любой формат телефона) | да — контакт для связи, войти по нему нельзя |
| login | string, 3–20 символов, латиница/цифры/`_` | да — придумывает сам пользователь, должен быть уникален |
| password | string, минимум 6 символов | да — придумывает сам пользователь |
...
```json
{
  ...
  "userId": "5e2a1e0a-1b1a-4c9e-9f0a-3a0f6c1e2b10",
  "login": "ivan_ivanov"
}
```
Ошибки: `409` — этот `login` уже занят; `422` — `login`/`password` не проходят формат (см. таблицу выше) или другие поля пустые/некорректные.
...
Body:
```json
{ "login": "ivan_ivanov", "password": "myS3cret" }
```
`login`/`password` — те же значения, что пользователь придумал при регистрации (`POST /api/auth/register`). У сид-админа `login` для примера равен `77000000000` — это просто зафиксированное сид-значение, не признак того, что логин обязан выглядеть как телефон.
...
Создать пользователя напрямую от имени админа (например, для тестовых аккаунтов) — в отличие от самостоятельной регистрации (`POST /api/auth/register`), здесь `login`/`password` генерируются автоматически, не пользователем, и возвращаются в ответе.
Body:
```json
...
**Частичное обновление** — любые из `question`, `option_a`-`option_d`, `correct`, `category`, `difficulty`, `is_active`.
Body (пример):
```json
{ "question": "Обновлённый текст вопроса?", "is_active": false }
```
200: обновлённый вопрос в `data`. Ошибки: `404` — вопрос не найден.

### `DELETE /api/admin/quiz/questions/:id`
Жёсткое удаление запрещено, если вопрос уже где-то используется — вместо того чтобы оставить висячую ссылку или упасть с невнятной ошибкой БД, эндпоинт заранее проверяет и объясняет причину:
- Есть ответы учеников на этот вопрос (`quiz_answers`) → `409`, удаление запрещено безусловно (историю ответов терять нельзя).
- Вопрос стоит в расписании квиза на одну или несколько дат (`daily_quiz_schedule`, включая будущие) → `409`, сначала замени его в расписании через `POST /api/admin/quiz/schedule`.
- Ничего не ссылается → удаляется по-настоящему.

В обоих случаях `409` в сообщении явно советует альтернативу — `PUT .../:id` с `{ "is_active": false }` скрывает вопрос из `GET /api/admin/quiz/questions` и из авто-подбора (`findUnusedQuestions`), ничего не удаляя.

200:
```json
{ "success": true, "message": "Вопрос удалён" }
```
Ошибки: `404` — вопрос не найден; `409 CONFLICT` — вопрос используется (см. выше).
...
Жёсткое удаление запрещено, если по товару уже есть заявки (`shop_orders`, в любом статусе — pending/approved/delivered/rejected, история заказов не должна ссылаться в никуда). Вместо удаления используй `PUT .../:id` с `{ "is_active": false }` — товар пропадёт из `GET /api/shop`, но история заказов останется целой.

200:
```json
{ "success": true, "message": "Товар удалён" }
```
Ошибки: `404` — товар не найден; `409 CONFLICT` — по товару уже есть заявки.
...
- При самостоятельной регистрации (`POST /api/auth/register`) логин/пароль придумывает сам пользователь — передавать их вручную не нужно. Автогенерация логина/пароля осталась только для админских путей (`POST /api/admin/users`, `reset-password`) — там значения возвращаются в ответе, куратор передаёт их получателю вручную (автоотправки никуда нет).
